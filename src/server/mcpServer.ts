...
2. **Use \`fiber_tree${MODULE_SEPARATOR}find_all\` with \`select: ["mcpId", "name", "bounds"]\` + \`host${MODULE_SEPARATOR}tap\` with \`bounds.centerX\`/\`bounds.centerY\`** when you need a real OS gesture (e.g. to verify Pressable/TouchableOpacity feedback, or to test code that listens to native gesture responders). The bounds come back in physical pixels — pass them straight to \`host${MODULE_SEPARATOR}tap\`, no scaling. The \`select\` parameter also saves tokens by omitting heavy \`props\` from the response.
