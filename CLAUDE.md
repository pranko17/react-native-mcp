...
- **Symbolication** (`host/tools/symbolicate.ts`): `host__symbolicate({ stack? | frames?, metroUrl?, maxFrames?, includeFrameworkFrames?, fullPaths? })` — POSTs to Metro's `/symbolicate` endpoint. Drops `collapse: true` framework frames by default, caps to 10 frames, shortens absolute paths relative to cwd. Graceful no-op `{ skipped: true, error, frames }` when Metro is unreachable.
