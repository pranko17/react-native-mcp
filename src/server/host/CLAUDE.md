...
`adb shell input text` only accepts ASCII — the underlying KeyCharacterMap has no entries for non-ASCII, and feeding it Cyrillic crashes the input service with `NullPointerException: Attempt to get length of null array`. Active IME layout is irrelevant: `input text` never reaches the IME. So `host__type_text` splits by charset — ASCII goes through `input text`, anything else is written to the device clipboard via `device__set_clipboard` (dispatched back into the connected app, the only side that can write it) and delivered with `KEYCODE_PASTE`. That keeps it a genuine system paste, so the field's own handlers run. Without a connected app the non-ASCII path has no way to fill the clipboard and returns an actionable error. Either way `host__type_text` does select-all + delete first (via `input keycombination 113 29` = Ctrl+A then `KEYCODE_DEL`), matching iOS replace-then-paste semantics.
...
- **`host__type_text`** — types into the currently-focused field after clearing it (select-all + delete). `submit: true` presses Enter after. Both platforms are layout-immune: iOS always pastes from the simulator clipboard, Android types ASCII via adb and pastes non-ASCII from the device clipboard. Nothing focused = the keystrokes go nowhere and the call still reports `typed: true` — tap the field first.
